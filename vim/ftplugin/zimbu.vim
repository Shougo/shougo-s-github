...
" braces in comments.
let b:match_words = '\(^\s*\)\@<=\(MODULE\|CLASS\|INTERFACE\|BITS\|ENUM\|SHARED\|FUNC\|REPLACE\|DEFINE\|PROC\|NEW\|EQUAL\|MAIN\|IF\|GENERATE_IF\|WHILE\|REPEAT\|DO\|FOR\|SWITCH\)\>\|{\s*$:\(^\s*\)\@<=\(ELSE\|ELSEIF\|GENERATE_ELSE\|GENERATE_ELSEIF\)\>:\(^\s*\)\@<=}\|\<UNTIL\>'
...
" Does replace when a dot is typed.
func! GCUpperDot(what)
  ...
  if synIDattr(synID(line("."), col(".") - 2, 1), "name") =~ 'Comment\|String'
    " inside a comment
    let g:got_char = 777
    return a:what
  endif
